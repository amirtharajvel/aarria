...
public class Util {
	...
	public static String messageTemplate(Order order, String status) {
		StringBuilder sb = new StringBuilder();
		if (CollectionUtils.isNotEmpty(order.getOrderedItems())) {
			int size = order.getOrderedItems().size();
			double total = Util.getTotalOrderAmount(order);

			String name = getProductName(order.getOrderedItems().get(0));
			String orderId = " with order id " + order.getOrderId();
			String amount = " for the amount " + total;
			...
			if (size == 2) {
				sb.append(name);
				sb.append(" and " + getProductName(order.getOrderedItems().get(1)));
				sb.append(orderId);
				sb.append(amount);
				sb.append(" is " + status);
			}
			...
			if (size > 2) {
				sb.append(name + " and " + " other " + (order.getOrderedItems().size() - 1) + " items");
				sb.append(orderId);
				sb.append(amount);
				sb.append(" is " + status);
			}
