...
public class CartServiceImpl implements CartService {
	...
	public CheckoutResponseDto prepareCart(User user, ModelAndView view, String couponCode) {
		...
		Map<String, ApplyOfferHelperDto> offers = new HashMap<>();
		if (fromCartPage != null && !fromCartPage.isEmpty()) {
			for (int i = 0; i < fromCartPage.size(); i++) {
				...
				double priceTobeAdded = cartDto.getPrice();
		...
		view.addObject("savings", offerService.calculateSavings(total, totalQuantity));

		if (couponCode != null) {
				dto.setCouponCodeAppliedMessage(offerService.isValidCouponCode(total, couponCode));
		} else {
